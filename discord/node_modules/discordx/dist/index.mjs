...
import { ApplicationCommandType as ApplicationCommandType3, SlashCommandBuilder } from "discord.js";
...
`
    ...
    this._events.sort(
      (a, b) => `${a.event}__${a.priority.toString()}` > `${b.event}__${b.priority.toString()}` ? 1 : -1
    );
  }
  buildSimpleCommands() {
    this._simpleCommands.forEach((cmd) => {
      if (cmd.prefix) {
        toStringArray(cmd.prefix).forEach(
          (pfx) => this._simpleCommandMappedPrefix.add(pfx)
        );
      }
